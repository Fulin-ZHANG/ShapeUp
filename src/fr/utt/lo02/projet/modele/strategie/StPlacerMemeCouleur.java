...
/**
 * The class {@code StPlacerMemCouleur} is used to play a card that has the
 * same color as the victory card near any card which has the same color as this
 ...
 * @version 1.0
 */
public class StPlacerMemeCouleur implements StrategiePlacer {
	 ...
	 * If the color is the same, then play this card on the table 
	 * in any direction possible of a card which has the same color 
	...
	public boolean play(Carte carteVictoire, Carte cartePlacer, Disposition disposition) {
		if (disposition.cartesTapis.isEmpty()) {
			...
			return true;
		} else if (carteVictoire.getCouleur() == cartePlacer.getCouleur()) { // 如果要放置的牌的颜色与胜利牌颜色一样
			...
			for (int i = 0; i < 5; i++) {
				for (int j = 0; j < 5; j++) {
					...
					if (carte != null && cartePlacer.getCouleur() == carte.getCouleur()) { // 选择颜色相同的牌
